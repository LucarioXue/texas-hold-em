...
export default function DecisionPanel() {
  ...
  const analysis = useMemo(() => {
    ...
    if (stage === 'preflop') {
      const equity = getPreflopEquity(h[0], h[1], analysisPlayerCount)
    ...
    const result = runSimulation(h, commFilled, analysisPlayerCount, 4000)
  ...
  return (
    <GlassPanel className="px-4 py-4">
      ...
      {/* Post-flop detail: collapsible outs + hand distribution */}
      {stage === 'postflop' && (analysis.outsInfo || analysis.handDistribution) && (
        <>
          <button
            onClick={() => setDetailExpanded(v => !v)}
            className="mt-3 flex items-center gap-1 text-white/40 hover:text-white/60 transition-colors"
          >
